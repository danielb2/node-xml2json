...
function startElement(name, attrs) {
    ...
    var forcedArray = false;

    if (! (name in currentObject)) {
        if (options.forceArray && options.forceArray.indexOf(name) !== -1) {
            forcedArray = true;
        }
        if(options.arrayNotation || forcedArray) {
...
module.exports = function(xml, _options) {
    ...
    var schema = {
        ...
        forceArray: joi.array().items(joi.string()).allow(null).default(null),
